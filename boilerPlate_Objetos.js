...
// saldo es: XXXX”.
  let banco =
  {
    ...
    {
        return this.clientes.find(cliente =>cliente.titularCuenta ===titular);
    },

    deposito: function (titular,deposito) 
    {
        let cliente = this.consultarCliente(titular)
         return cliente.saldoEnPesos+=deposito;
    ...
    extraccion: function(titular,montoextraer)
    {
        ...
        {
            return `Fondos Insuficientes, Saldo: ${cliente.saldoEnPesos}`;
        }
        else{
            return `Extracción realizada correctamente, su nuevosaldo es: ${cliente.saldoEnPesos}`;
        }
    }
  }
    // let nuevosaldo =banco.deposito("Jacki Shurmer",20000) ;
    // console.log(`Depósito realizado, su
    // nuevo saldo es: ${nuevosaldo}`);

// Encuentra un cliente    
let clienteEncontrado = banco.consultarCliente('Jacki Shurmer');
console.log(clienteEncontrado);

// Deposito y nuevo Saldo
let nuevo_saldo=banco.deposito("Jarret Lafuente",10);
console.log(`Su Nuevo Saldo es de: ${nuevo_saldo}`);

// Retiro y Nuevo Saldo
let extraccion_real=banco.extraccion("Ansel Ardley",100);
console.log(extraccion_real);